...
extension FeedbackView: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didHighlightRowAt indexPath: IndexPath) {
        ...
        // swiftlint:disable:next force_cast
        feedbackService.addFeedback(itemName: currentItem?.value(forKey: "name") as! String, feedback: feedback).startWithCompleted {
            self.view.makeToast("Feedback submitted", duration: TimeInterval.abs(1), position: ToastPosition.bottom)
        }
    }

    func reloadFeedbackCaptureView(item optionalItem: NSManagedObject?) {
        if let item = optionalItem {
            ...
            itemImage.image?.accessibilityLabel = ""
        }
        currentItem = optionalItem
